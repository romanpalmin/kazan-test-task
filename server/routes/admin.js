...
// Голый /admin (без .html) естественно набрать по привычке — но это не
// статика, а маунт-точка API-роутера, и без этого редиректа он молча
// падал бы в токен-мидлвар ниже с криптичным 401 вместо того, чтобы
// показать саму страницу. Регистрируется ДО токен-мидлвара специально,
// чтобы сама переадресация не требовала токена (сама страница ничего
// не покажет без токена всё равно). Относительный путь ('admin.html',
// без ведущего слэша) — намеренно: приложение может быть смонтировано
// не в корне (например, за nginx-прокси на /kazan-test-task/), и
// абсолютный '/admin.html' увёл бы браузер в настоящий корень сайта,
// а не в корень этого приложения. Location-заголовок с относительной
// ссылкой браузер резолвит от текущего URL запроса — работает
// одинаково что в корне, что под под-путём.
router.get('/', (req, res) => res.redirect('admin.html'));
